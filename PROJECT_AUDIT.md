...
Gaps identified here: `ALLOWED_ORIGINS` defaults to `*`; no dependency vulnerability scanning; no documented input-validation coverage audit; unverified refresh-token flow. No secrets manager — Railway dashboard env vars are the only secret store, which is fine but should be explicit in DEPLOYMENT.md.

**Update (2026-08-18, Priority 2 pass — see `PROJECT_STATUS.md` for full evidence):** every gap above has since been either resolved or turned into an explicit, documented owner decision. Dependency vulnerabilities: fixed (Priority 1). Refresh-token flow: confirmed unnecessary for this app and removed. CORS: enforcement mechanism now tested (8 tests); the `*` policy choice itself is still an open owner decision, not an oversight. Input validation: risk-based audit completed across all 24 previously-unvalidated route files; highest-risk gaps (price alerts, brain follows, guide sell-now) fixed and tested; one new finding (Telegram webhook has no authenticity check) queued as `TASKS.md` T-020 for owner decision. This paragraph is left in place as the original point-in-time finding; treat `PROJECT_STATUS.md` as the live source of truth going forward.
