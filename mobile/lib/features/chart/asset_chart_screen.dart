...
class AssetChartScreen extends ConsumerStatefulWidget {
  ...
  const AssetChartScreen({super.key, required this.asset, this.signal, this.signalConf});
...
class _AssetChartScreenState extends ConsumerState<AssetChartScreen> {
  ...
  String get _interval => switch (_tf) { '4H' => '4h', '1D' => '1d', _ => '1h' };
  int    get _limit    => switch (_tf) { '4H' => 42,   '1D' => 60,   _ => 48   };

  @override
  Widget build(BuildContext context) {
    final query      = ChartQuery(asset: widget.asset, interval: _interval, limit: _limit);
    final chartAsync = ref.watch(chartProvider(query));
    final brainAsync = ref.watch(brainActionProvider);

    ActionReport? report;
    brainAsync.whenData((r) { if (r.bestAsset == widget.asset) report = r; });
      ...
      body: chartAsync.when(
        ...
        data: (candles) {
          ...
          final tc        = (_touched != null && _touched! < candles.length)
              ? candles[_touched!] : null;
          final last      = candles.last;
          final pctChange = (last.close - candles.first.close) / candles.first.close * 100;
          ...
          final lastRsi   = candles.lastWhere((c) => c.rsi != null, orElse: () => last).rsi;
          ...
          return Column(children: [
            ...
            Padding(
              ...
              child: Row(children: [
                ...
                if (hasEma20) ...[
                  ...
                  _LegendDot(color: const Color(0xFFF7931A), label: 'EMA20', dashed: true),
                ],
                if (hasEma50) ...[
                  const SizedBox(width: 10),
                  _LegendDot(color: const Color(0xFF9945FF), label: 'EMA50', dashed: true),
                ],
                const Spacer(),
                // Time selector right-aligned
                _TimeSelector(
                  current:   _tf,
                  onChanged: (v) => setState(() { _tf = v; _touched = null; }),
                ),
              ]),
            ),
            ...
            if (hasRsi) ...[
              Padding(
                ...
                child: Row(children: [
                  const Text('RSI', style: TextStyle(
                  ...
                  if (lastRsi != null)
                    Text(lastRsi.toStringAsFixed(0),
                        style: TextStyle(
                            ...
                            color: lastRsi > 70 ? AppColors.sell
                                : lastRsi < 30 ? AppColors.buy
                                : AppColors.textMuted)),
                ]),
              ),
              Padding(
...
class _PriceChart extends StatelessWidget {
  ...
  const _PriceChart({required this.candles, required this.touched, required this.onTouch});
  ...
  Widget build(BuildContext context) {
    ...
    final spots = candles.asMap().entries
        .map((e) => FlSpot(e.key.toDouble(), e.value.close))
        .toList();

    final ema20Spots = candles.asMap().entries
        .where((e) => e.value.ema20 != null)
        .map((e) => FlSpot(e.key.toDouble(), e.value.ema20!))
        .toList();

    final ema50Spots = candles.asMap().entries
      ...
      LineChartData(
        ...
        gridData: FlGridData(
          ...
          getDrawingHorizontalLine: (_) =>
              FlLine(color: AppColors.border.withValues(alpha: 0.35), strokeWidth: 0.5),
        ),
        ...
        lineTouchData: LineTouchData(
          ...
          getTouchedSpotIndicator: (_, spotIndexes) => spotIndexes.map((_) =>
            TouchedSpotIndicatorData(
              FlLine(color: AppColors.textMuted.withValues(alpha: 0.5), strokeWidth: 1),
  ...
  Widget build(BuildContext context) {
    ...
    final groups = candles.asMap().entries.map((e) {
      ...
      return BarChartGroupData(
        x: e.key,
        barRods: [BarChartRodData(toY: c.volume, color: col, width: 2,
    ...
    return LineChart(LineChartData(
      ...
      gridData: FlGridData(
        ...
        getDrawingHorizontalLine: (v) => FlLine(
          color: (v == 70 ? AppColors.sell : AppColors.buy).withValues(alpha: 0.25),
          strokeWidth: 0.8,
          dashArray: [4, 4],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (report?.entryPrice != null || report?.stopLoss != null ||
          ...
          Row(children: [
            if (report!.entryPrice  != null)
              Expanded(child: _Level('Entry',  report!.entryPrice!, AppColors.primary)),
            if (report!.stopLoss    != null)
              Expanded(child: _Level('Stop',   report!.stopLoss!,   AppColors.sell)),
            if (report!.takeProfit  != null)
              Expanded(child: _Level('Target', report!.takeProfit!, AppColors.buy)),
          ]),
        ],
      ]),
    );
...
class _Level extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(children: [
    ...
    Text(_fmtPrice(value), style: TextStyle(
...
class _AssetDot extends StatelessWidget {
  ...
  static const _colors = [
    ...
    Color(0xFFBA9F33), Color(0xFFE84142), Color(0xFF2A5ADA), Color(0xFF8247E5),
  ];
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Center(child: Text(
        ...
        style: TextStyle(color: color, fontSize: size * 0.3, fontWeight: FontWeight.w800),
      )),
    );
  }
}
